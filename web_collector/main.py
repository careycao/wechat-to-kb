    ...
    try:
        for index, raw_url in enumerate(urls, 1):
            ...
            if not url or url.startswith("#"):
                continue
            if _is_wechat_article(url):
                logger.warning("检测到公众号文章，请改用 wechat_collector: %s", redact_url_for_log(url))
                continue
                ...
                if kb_hint and kb_hint in writers:
                    ...
                    if kb_key is None:
                        if auto_route:
                            ...
                            print(
                                f"[web_collector] 置信度低，已自动选用得分最高：{kb.name} / {prefixed}",
                                file=sys.stderr,
                            )
                        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="web_collector — 普通网页入库工具")
    parser.add_argument("url", nargs="*", help="网页 URL，可传多个")
